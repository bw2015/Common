            ...
            for (let type in options) {
                switch (type) {
                    ...
                    default:
                        ...
                        for (var key in item) {
                            if (key.indexOf("-") != -1 || key == "style") html.push(" " + key + "='" + item[key] + "'");
                            else html.push(" data-" + key + "='" + item[key] + "'");
                        }
...
!function (ns) {
    ...
    ns["number"] = function (value, dec) {
        ...
        let num = typeof value === "string" ? Number(value) : value,
            integer = num * Math.pow(10, dec);
        num = Math.floor(num * Math.pow(10, dec));
        // 解决js 浮点转整数的问题
        if (num !== integer && integer - num > 0.9999) {
            num++;
        }
        ...
        var arr = [];
        // 小数部分
        var decimal = (Math.abs(num) % Math.pow(10, dec)).toString();
        ...
        var numString = Math.floor(Math.abs(num) / Math.pow(10, dec)).toString();
        ...
        return negative + arr.join(",") + (dec ? "." + decimal.padLeft(dec, '0') : "");
    };
