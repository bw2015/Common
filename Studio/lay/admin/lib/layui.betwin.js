layui.define(['form', 'table', 'admin', 'laytpl', 'laydate', 'view', "upload"], function (exports) {
    ...
    betwin.form = {
            ...
            form.on("submit(" + filter + ")", function (e) {
                ...
                if (request.tokenName) {
                    ...
                    if (request.getToken && typeof request.getToken === "function") {
                        ...
                        headers[tokenName] = layui.data(setter.tableName)[tokenName] || '';
                    }
                }
        ...
        render: function (filter) {
            ...
            // 配置枚举
            !function () {
                formElem.querySelectorAll("select[data-enum]").forEach(select => {
                    let name = select.getAttribute("data-enum"),
                        value = select.getAttribute("data-value");
                    if (!GolbalSetting.enum || !GolbalSetting.enum[name]) {
                        console.error(`找不到枚举${name}`);
                        return;
                    }
                    if (GolbalSetting.enum[name] instanceof Map) {
                        GolbalSetting.enum[name].forEach((text, key) => {
                            select.options.add(new Option(text, key));
                        });
                    }
